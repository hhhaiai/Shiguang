...
- 2026-02-23: 分享渲染链路稳定性更新  
  - 分享图渲染宽度对齐手机物理屏幕宽度，修复异常缩放与超高截断问题  
  - 图片与 Mermaid 在预览/分享页按中线展示，视觉对齐优化  
  - HTML `details` 在分享模式改为静态渲染，避免折叠组件本地化依赖导致红屏  
  - Draw.io 代码块在当前阶段使用安全占位文案（渲染暂禁），避免超长 XML 影响分享图生成
