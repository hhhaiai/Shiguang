...
+ (instancetype)initWithParams:(NSDictionary *)params onProgress:(void (^)(unsigned int progress))onProgress {
    ...
    NSLog(@"[fllama] FLlamaContext.initWithParams enter");

    NSString *modelPath = params[@"model"];
    BOOL isAsset = [params[@"is_model_asset"] boolValue];
    NSString *path = modelPath;
    NSLog(@"[fllama] FLlamaContext modelPath=%@ isAsset=%d", modelPath, isAsset);

    if (isAsset) {
        path = [[NSBundle mainBundle] pathForResource:modelPath ofType:nil];
    }
    NSLog(@"[fllama] FLlamaContext resolvedPath=%@", path);
    if (path == nil || [path length] == 0) {
        NSLog(@"[fllama] FLlamaContext invalid model path");
        return nil;
    }

    defaultParams.model = [path UTF8String];
    NSLog(@"[fllama] FLlamaContext defaultParams.model assigned");
    ...
    NSString *loraPath = params[@"lora"];
    if ([loraPath isKindOfClass:[NSString class]] && [loraPath length] > 0) {
        ...
        defaultParams.lora_adapters.push_back({ [loraPath UTF8String], lora_scaled });
    ...
    // common_params contains raw callback pointers - ensure deterministic
    // defaults when progress callback is not requested.
    defaultParams.progress_callback = nullptr;
    defaultParams.progress_callback_user_data = nullptr;
    ...
    NSLog(@"[fllama] FLlamaContext threads=%d n_ctx=%d n_batch=%d mmap=%d",
          defaultParams.cpuparams.n_threads,
          defaultParams.n_ctx,
          defaultParams.n_batch,
          defaultParams.use_mmap);

    FLlamaContext *context = [[FLlamaContext alloc] init];
    NSLog(@"[fllama] FLlamaContext object allocated");
    context->llama = new fllama::fllama_context();
    NSLog(@"[fllama] FLlamaContext llama context allocated");
    context->llama->is_load_interrupted = false;
    context->llama->loading_progress = 0;
    context->onProgress = [onProgress copy];
    ...
    NSLog(@"[fllama] FLlamaContext loadModel begin");
    context->is_model_loaded = context->llama->loadModel(defaultParams);
    NSLog(@"[fllama] FLlamaContext loadModel end loaded=%d", context->is_model_loaded);
...
- (NSString *)getFormattedChat:(NSArray *)messages withTemplate:(NSString *)chatTemplate {
    ...
    for (NSDictionary *msg in messages) {
        id roleObj = [msg objectForKey:@"role"];
        id contentObj = [msg objectForKey:@"content"];
        NSString *roleStr = [roleObj isKindOfClass:[NSString class]] ? roleObj : @"user";
        NSString *contentStr = [contentObj isKindOfClass:[NSString class]] ? contentObj : @"";
        std::string role = [roleStr UTF8String];
        std::string content = [contentStr UTF8String];
        chat.push_back({ role, content });
    }

    NSString *templateStr = [chatTemplate isKindOfClass:[NSString class]] ? chatTemplate : @"";
    auto tmpl = [templateStr UTF8String];
