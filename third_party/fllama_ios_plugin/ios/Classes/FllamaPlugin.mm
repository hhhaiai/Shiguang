...
static NSDictionary *FllamaExceptionDetails(NSException *exception) {
    NSString *name = exception.name ?: @"NSException";
    NSString *reason = exception.reason ?: @"Unknown native exception";
    return @{
        @"name": name,
        @"reason": reason,
    };
...
- (void)handleInitContext:(FlutterMethodCall *)call result:(FlutterResult)result {
    @try {
        ...
        NSLog(@"[fllama] initContext begin");
        ...
        NSLog(@"[fllama] initContext loaded=%d", [context isModelLoaded]);
        ...
        // Keep init payload minimal; large model metadata can destabilize
        // method-channel serialization on some simulator environments.
        NSLog(@"[fllama] initContext return contextId=%@", contextIdNumber);
        result(@{ @"contextId": contextIdNumber });
    } @catch (NSException *exception) {
        result([FlutterError errorWithCode:@"505" message:exception.reason details:FllamaExceptionDetails(exception)]);
    }
}
    ...
    @try {
        ...
        dispatch_async(llamaDQueue, ^{
            @try {
                ...
                result([FlutterError errorWithCode:@"500" message:exception.reason details:FllamaExceptionDetails(exception)]);
            }
        });
    } @catch (NSException *exception) {
        result([FlutterError errorWithCode:@"505" message:exception.reason details:FllamaExceptionDetails(exception)]);
